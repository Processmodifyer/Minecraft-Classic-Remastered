...
{
    protected Item(int id, int textureID) {
		super(id, textureID);
	}
	private Player player;
    public boolean onAdded;
	public Minecraft minecraft;
	public static int itemTextureID;
	public int item;
    ...
    public void onAdded(final World level, final int x, final int y, final int z) {
        level.setTile(x, y, z, 0);
            if (this.onAdded) {
            	player.inventory.DonotRemoveResource(item);
      }
    ...
    public final void renderPreview(final ShapeRenderer var1) {
        ...
        var1.begin();
        this.render(var1, 0.0f, 0.5f, -0.3f);
        var1.end();
    }
    private void render(final ShapeRenderer var1, final float var2, final float var3, final float var4) {
        ...
        for (int var12 = 0; var12 < 2; ++var12) {
            float var13 = (float)(MathHelper.sin(var12 * 3.1415927f / 1.0f + 0.7853982f) * 0.5);
