package net.classic.remastered.game.world.generator;

import net.classic.remastered.game.world.World;
import net.classic.remastered.game.world.ProgressBarDisplay;
import net.classic.remastered.game.world.generator.noise.WorldCombinedNoise;
import net.classic.remastered.game.world.generator.noise.WorldOctaveNoise;
...
public class WorldGenerator {
   ...
   public WorldGenerator(ProgressBarDisplay var1) {
      this.progressBar = var1;
   }

   public World generate(String var1, int var2, int var3, int var4) {
      ...
      WorldCombinedNoise var6 = new WorldCombinedNoise(new WorldOctaveNoise(this.random, 8), new WorldOctaveNoise(this.random, 8));
      WorldCombinedNoise var7 = new WorldCombinedNoise(new WorldOctaveNoise(this.random, 8), new WorldOctaveNoise(this.random, 8));
      WorldOctaveNoise var8 = new WorldOctaveNoise(this.random, 6);
      ...
      int[] var42 = var9;
      var5 = this;
      var7 = new WorldCombinedNoise(new WorldOctaveNoise(this.random, 8), new WorldOctaveNoise(this.random, 8));
      WorldCombinedNoise var49 = new WorldCombinedNoise(new WorldOctaveNoise(this.random, 8), new WorldOctaveNoise(this.random, 8));
      ...
      var51 = this.height;
      WorldOctaveNoise var53 = new WorldOctaveNoise(this.random, 8);
      ...
      this.populateOre(Block.REDSTONE_ORE.id, 57, 3, 4);
      ...
      var48 = this.depth;
      var51 = this.height;
      var53 = new WorldOctaveNoise(this.random, 8);
      WorldOctaveNoise var58 = new WorldOctaveNoise(this.random, 8);
      ...
      World var65;
      (var65 = new World()).waterLevel = this.waterLevel;
   ...
   public void GenerateWorkBench() {
       final int xSpawn = World.xSpawn;
       final int ySpawn = World.ySpawn;
       final int zSpawn = World.zSpawn;
       for (int i = xSpawn - 3; i <= xSpawn + 3; ++i) {
           for (int j = ySpawn - 2; j <= ySpawn + 2; ++j) {
               for (int k = zSpawn - 3; k <= zSpawn + 3; ++k) {
                   if (i == xSpawn - 3 || k == zSpawn - 3 || i == xSpawn + 3 || k == zSpawn + 3 || j == ySpawn - 2 || j == ySpawn + 2) {
                       if (j >= ySpawn - 1) {
                       }
                   }
                   if (k == zSpawn - 3 && i == xSpawn && j >= ySpawn - 1 && j <= ySpawn) {
                   }
               }
           }
       }
